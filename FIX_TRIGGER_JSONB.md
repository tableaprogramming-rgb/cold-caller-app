# Fix: Trigger Function jsonb Operator Error

## Issue
PostgreSQL error: `operator does not exist: jsonb - jsonb`

The trigger function uses `jsonb_agg()` which returns a jsonb array, but PostgreSQL's `-` operator on jsonb only accepts text or text arrays, not jsonb.

## Solution
Run this SQL in Supabase SQL Editor to recreate the fixed trigger function:

```sql
-- Drop old trigger to release the function
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- Remove null entries (FIXED: use array_agg()::text[] instead of jsonb_agg())
...
4. Run it
5. Try changing a contact status again — should work now
...
The key difference: `array_agg(key)::text[]` produces a text array, which the `-` operator accepts.
